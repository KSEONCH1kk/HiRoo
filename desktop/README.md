...
- ✨ Splash-экран с прогрессом установки обновлений.
- 🔄 Авто-обновление: проверка `/api/desktop/latest` при запуске, silent install (`Setup.exe /S` на Windows).
...
- ⌨️ Глобальные хоткеи через `uiohook-napi`: настоящий push-to-talk с hold/release, пассивно (не блокирует клавишу в других приложениях), работает когда окно свёрнуто.
